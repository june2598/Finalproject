...
public class TrendDAOImpl implements TrendDAO {
  ...
  public List<SectorsTrendRateDto> getSectorTrend(int marketId, String orderBy) {
    ...
    sql.append("         SELECT ");
    sql.append("         SEC_NM,  ");
    sql.append("         SUM(CASE WHEN SOURCE = 'NEWS' AND PUBLISHED_DATE >= TRUNC(SYSDATE - 2) + 9/24 AND PUBLISHED_DATE < TRUNC(SYSDATE - 1) + 9/24 THEN 1 ELSE 0 END) AS total_yesterday_news_count, ");
    sql.append("         SUM(CASE WHEN SOURCE = 'NEWS' AND PUBLISHED_DATE >= TRUNC(SYSDATE - 1) + 9/24 AND PUBLISHED_DATE < TRUNC(SYSDATE) + 9/24 THEN 1 ELSE 0 END) AS total_today_news_count, ");
    sql.append("         SUM(CASE WHEN SOURCE = 'COMMUNITY' AND PUBLISHED_DATE >= TRUNC(SYSDATE - 2) + 9/24 AND PUBLISHED_DATE < TRUNC(SYSDATE - 1) + 9/24 THEN 1 ELSE 0 END) AS total_yesterday_community_count, ");
    sql.append("         SUM(CASE WHEN SOURCE = 'COMMUNITY' AND PUBLISHED_DATE >= TRUNC(SYSDATE - 1) + 9/24 AND PUBLISHED_DATE < TRUNC(SYSDATE) + 9/24 THEN 1 ELSE 0 END) AS total_today_community_count ");
    sql.append("         FROM ");
    sql.append("             ( ");
    sql.append("                 SELECT ");
    sql.append("                 C.SEC_NM, ");
  ...
  public List<StocksTrendRateDto> getStocksTrend(String orderBy) {
    ...
    sql.append(" ROUND( ");
    sql.append("     CASE ");
    sql.append("     WHEN (SELECT COUNT(*) FROM NEWS WHERE PUBLISHED_DATE >= TRUNC(SYSDATE - 2) + 9/24 AND PUBLISHED_DATE < TRUNC(SYSDATE - 1) + 9/24 AND STK_ID = C.STK_ID) = 0 THEN NULL ");
    sql.append(" ELSE ( ");
    sql.append("         (SELECT COUNT(*) FROM NEWS WHERE PUBLISHED_DATE >= TRUNC(SYSDATE - 1) + 9/24 AND PUBLISHED_DATE < TRUNC(SYSDATE) + 9/24 AND STK_ID = C.STK_ID) - ");
    sql.append("     (SELECT COUNT(*) FROM NEWS WHERE PUBLISHED_DATE >= TRUNC(SYSDATE - 2) + 9/24 AND PUBLISHED_DATE < TRUNC(SYSDATE - 1) + 9/24 AND STK_ID = C.STK_ID) ");
    sql.append("         ) / (SELECT COUNT(*) FROM NEWS WHERE PUBLISHED_DATE >= TRUNC(SYSDATE - 2) + 9/24 AND PUBLISHED_DATE < TRUNC(SYSDATE - 1) + 9/24 AND STK_ID = C.STK_ID) * 100 ");
    sql.append(" END, 2) AS NEWS_INCREASE_RATE, ");
    sql.append(" ROUND( ");
    sql.append("     CASE ");
    sql.append("     WHEN (SELECT COUNT(*) FROM COMMUNITY WHERE POST_DATE >= TRUNC(SYSDATE - 2) + 9/24 AND POST_DATE < TRUNC(SYSDATE - 1) + 9/24 AND STK_ID = C.STK_ID) = 0 THEN NULL ");
    sql.append(" ELSE ( ");
    sql.append("         (SELECT COUNT(*) FROM COMMUNITY WHERE POST_DATE >= TRUNC(SYSDATE - 1) + 9/24 AND POST_DATE < TRUNC(SYSDATE) + 9/24 AND STK_ID = C.STK_ID) - ");
    sql.append("     (SELECT COUNT(*) FROM COMMUNITY WHERE POST_DATE >= TRUNC(SYSDATE - 2) + 9/24 AND POST_DATE < TRUNC(SYSDATE - 1) + 9/24 AND STK_ID = C.STK_ID) ");
    sql.append("         ) / (SELECT COUNT(*) FROM COMMUNITY WHERE POST_DATE >= TRUNC(SYSDATE - 2) + 9/24 AND POST_DATE < TRUNC(SYSDATE - 1) + 9/24 AND STK_ID = C.STK_ID) * 100 ");
    sql.append(" END, 2) AS COMMUNITY_INCREASE_RATE ");
    sql.append(" FROM ");
    sql.append("     ( ");
